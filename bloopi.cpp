...
#undef HACK
	...
	options::options_description opts{"Bloopi interface server options"};
	// clang-format off
	opts.add_options()
	...
	info("starting bloopi hub {}", std::string("a string"));

	BloopiHub bloopi(oscDstAddr, oscDstPort, oscRcvPort, wsPort, threadCount);
	bloopi.run();
#endif
	return 0;
}